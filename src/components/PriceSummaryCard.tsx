...
export const PriceSummaryCard = ({
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-panel/80 p-6 shadow-luxe backdrop-blur xl:p-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        <button
          type="button"
          onClick={onRefresh}
          className="inline-flex h-11 items-center justify-center rounded-full border border-white/15 px-5 text-sm font-semibold text-white transition hover:border-accent/50 hover:bg-white/5"
        >
          시세 새로고침
        </button>
      </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-[1.4fr_1fr_1fr]">
        <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-5">
          <p className="text-sm text-white/55">24K 실시간 기준가 · 3.75g</p>
          {isLoading ? (
        ...
        <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-5">
          <p className="text-sm text-white/55">24K 1g 기준</p>
          {isLoading ? (
            <div className="mt-3 h-8 animate-pulse rounded-2xl bg-white/10" />
          ) : (
        ...
        <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-5">
          <p className="text-sm text-white/55">마지막 업데이트</p>
          {isLoading ? (
            <div className="mt-3 h-8 animate-pulse rounded-2xl bg-white/10" />
          ) : (
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3 text-sm">
        {data && (
          <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-white/65">
            제공처: {data.provider}
          </span>
        )}
        {error && (
